#define ana_delphes_2500_cxx
#include "ana_delphes_2500.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>

void ana_delphes_2500::Loop()
{
//   In a ROOT session, you can do:
//      Root > .L ana_delphes_2500.C
//      Root > ana_delphes_2500 t
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
      ...
      for(int i=0; i<Particle_size; i++){
         ...
         if((abs(PID)==1 || abs(PID)==2 || abs(PID)==3 || abs(PID)==4 || abs(PID)==5 || abs(PID)==21) && Status==3) HT+=Particle_PT[i];
